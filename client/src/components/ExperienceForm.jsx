...
const ExperienceForm = ({ data, onChange }) => {
  ...
  const addExperience = () => {
    ...
    onChange([...data, newExperience]); // Fixed: handle undefined data
  };

  const removeExperience = (index) => {
    const updated = (data || []).filter((_, i) => i !== index); // Also fixed here for consistency
    onChange(updated);
  };

  const updateExperience = (index, field, value) => {
    const updated = [...(data || [])]; // Fixed: handle undefined data
  ...
  const generateDescription = async (index) => {
    ...
    const prompt = `enhnace the job description ${experience.description} for the position of ${experience.position} at ${experience.company}.`;
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      {(data || []).length === 0 ? ( // Fixed: handle undefined data
        ...
        <div className="space-y-4">
          {(data || []).map(
            (
              exp,
              index // Fixed: handle undefined data
            ) => (
              <div
                key={index}
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full text-sm px-3 py-2 rounded-lg border border-gray-300 resize-none min-h-[100px]"
